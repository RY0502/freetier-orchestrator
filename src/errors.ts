...
export function defaultClassify(error: unknown): ErrorKind {
  const status = statusOf(error);

  if (status !== undefined) {
    if (QUOTA_STATUS.has(status)) return ErrorKind.Quota;
    if (RETRYABLE_STATUS.has(status)) return ErrorKind.Retryable;
  }

  const message = messageOf(error);

  if (QUOTA_PATTERNS.some((pattern) => pattern.test(message))) return ErrorKind.Quota;
  if (RETRYABLE_PATTERNS.some((pattern) => pattern.test(message))) return ErrorKind.Retryable;

  return ErrorKind.Fatal;
}

/** Classify an error, allowing the provider to override the default behavior. */
export function classifyError(error: unknown, provider: Provider<never, never>): ErrorKind {
