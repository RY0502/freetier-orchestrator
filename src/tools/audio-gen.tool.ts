...
export function createAudioGenTool(options?: {
  ...
  if (apiKeys.length === 0) {
    throw new Error(
      "[GroqAudio] No Groq API keys found. Set GROQ_API_KEY_1, GROQ_API_KEY_2, etc. in your environment."
    );
  }
  ...
  return new DynamicStructuredTool({
    ...
    func: async (toolInput) => {
        ...
        for (let attempt = 0; attempt <= TRANSIENT_MAX_RETRIES; attempt++) {
          try {
            ...
            if (isQuotaError(error)) {
              logger.warn(
                `[GroqAudio] Quota exhausted on key ${keyIndex + 1}/${apiKeys.length}: ${lastError.message}. ` +
            ...
            if (isTransientError(error) && attempt < TRANSIENT_MAX_RETRIES) {
              logger.warn(
                `[GroqAudio] Transient error on attempt ${attempt + 1}/${TRANSIENT_MAX_RETRIES + 1}: ${lastError.message}. ` +
            ...
            if (isTransientError(error) && attempt >= TRANSIENT_MAX_RETRIES) {
              logger.warn(
                `[GroqAudio] Transient retries exhausted on key ${keyIndex + 1}. ` +
                  (keyIndex < apiKeys.length - 1 ? "Switching to next key." : "All keys exhausted.")
              );
              break;
            }
      ...
      const message =
        `[GroqAudio] Audio generation failed. All ${apiKeys.length} API key(s) exhausted. ` +
