...
export class PollinationsImageProvider {
    ...
    classifyError(error) {
        if (error instanceof HttpError) {
            ...
            if ([408, 500, 502, 503, 504, 529].includes(error.status))
                return ErrorKind.Retryable;
            return ErrorKind.Fatal;
        }
        const message = error instanceof Error ? error.message : String(error);
        if (/rate.?limit|too many requests|quota|429/i.test(message))
            return ErrorKind.Quota;
        ...
        for (let attempt = 0; attempt <= TRANSIENT_MAX_RETRIES; attempt++) {
            try {
                ...
                if (attempt < TRANSIENT_MAX_RETRIES) {
                    this.logger.warn(`[Pollinations] Transient error on attempt ${attempt + 1}/${TRANSIENT_MAX_RETRIES + 1}: ${lastError.message}. ` +
                        `Retrying in ${TRANSIENT_RETRY_DELAY_MS / 1000}s...`);
                    await sleep(TRANSIENT_RETRY_DELAY_MS);
                }
                else {
                    this.logger.error(`[Pollinations] All ${TRANSIENT_MAX_RETRIES + 1} attempts exhausted. Last error: ${lastError.message}`);
                }
            }
        }
        throw new Error(`[Pollinations] Image generation failed after ${TRANSIENT_MAX_RETRIES + 1} attempts. ` +
    ...
    buildUrl(encodedPrompt, input) {
        ...
        if (input.safe !== undefined)
            params.set("safe", String(input.safe));
        if (input.quality !== undefined)
            params.set("quality", input.quality);
        if (input.image !== undefined)
            params.set("image", input.image);
        if (input.transparent !== undefined)
            params.set("transparent", String(input.transparent));
        if (input.duration !== undefined)
            params.set("duration", String(input.duration));
        if (input.aspectRatio !== undefined)
            params.set("aspectRatio", input.aspectRatio);
        if (input.audio !== undefined)
            params.set("audio", String(input.audio));
    ...
    async saveImage(outputPath, imageBuffer) {
